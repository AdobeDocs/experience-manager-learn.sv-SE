...
+ __Orsak:__ Rörledningens målmiljö är i ett övergångstillstånd där den inte kan ta emot nya byggen.
...
Under byggfasen och enhetstestningsfasen utförs en Maven-bygge (`mvn clean package`) av projektet som checkas ut från pipelinens konfigurerade Git-gren.

Fel som upptäcks i den här fasen bör kunna reproduceras lokalt, med följande undantag:

+ Ett maven-beroende som inte finns tillgängligt för [Maven Central](https://search.maven.org/) används, och Maven-databasen som innehåller beroendet är antingen:
...
+ Enhetstester misslyckas på grund av timingproblem. Detta kan inträffa när enhetstester är tidskänsliga. En stark indikator bygger på `.sleep(..)` i testkoden.
...
Kodskanning resulterar i ett byggfel om det finns en allvarlig säkerhetslucka i koden. Mindre överträdelser kan åsidosättas, men vi rekommenderar att de åtgärdas. Observera att kodskanningen är ofullständig och kan resultera i [falskt positiva](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/developing/understand-test-results.html#dealing-with-false-positives)resultat.
...
Mer information finns AEM särskilda regler i Cloud Manager-dokumentationernas [anpassade AEM-specifika kodssökningsregler](https://docs.adobe.com/content/help/en/experience-manager-cloud-manager/using/how-to-use/custom-code-quality-rules.html).
...
+ Distributionsprogrammet uppdaterar Core Components maven-beroendeversionen i `core` OSGi bundle-projektet
...
+ __Symtom:__
Steget Skapa bild misslyckas med en FELrapportering som 
...
+ __Orsak:__  Programmets OSGi-paket (definieras i `core` projektet) importerar Java-klasser från kärnberoendet för kärnkomponenter, på en annan versionsnivå än vad som distribueras till AEM som en Cloud Service.
+ __Upplösning:__
...
Bekanta dig med [AEM som en Cloud Service loggar](./logs.md) innan du felsöker steget Distribuera till. Loggen innehåller information om hur du startar och stänger av fönster som kan vara relevanta för att distribuera vid problem. `aemerror` Observera att loggen som är tillgänglig via knappen Hämta logg i steget Distribuera till i Cloud Manager inte är `aemerror` loggen och inte innehåller detaljerad information som gäller för att starta program.
...
+ __Upplösning:__ Granska implementeringen av kod som körs tidigt i OSGi Bundles livscykel och granska `aemerror` loggarna för AEM Author och Publish services vid tidpunkten för felet (loggtid i GMT) enligt Cloud Manager, och sök efter loggmeddelanden som indikerar anpassade loggprocesser som körs.
...
+ __Upplösning:__ Granska loggarna `aemerror` för AEM Author och Publish services runt tiden (loggtid i GMT) för felet, vilket visas av Cloud Manager.
...
`/var` kan ändras och innehåller ett antal olika typer av transient runtime-innehåll. Inkludera `/var` i innehållspaket (t.ex. `ui.content`) som distribueras via Cloud Manager kan få distributionen att misslyckas.
...
1. Avgör att minst ett innehållspaket som ingår i distributionen skrivs till `/var`.
...
+ __Orsak:__ AEM som används för att distribuera innehållspaket till AEM Publish-tjänsten kan inte skriva till `/var` AEM Publish. Detta leder till att distributionen av innehållspaketet till AEM Publish-tjänsten misslyckas.
+ __Upplösning:__ Följande sätt att lösa problemet visas i prioritetsordning:
   1. Om `/var` resurserna inte behövs tar du bort resurser under `/var` från innehållspaket som distribueras som en del av programmet.
   2. Om `/var` resurserna behövs definierar du nodstrukturerna med [repopoint](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit). Repoinit-skript kan riktas mot AEM Author, AEM Publish eller båda, via OSGi-runmodes.
   3. Om `/var` resurserna bara krävs av AEM författare och inte kan modelleras med [repoinit](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit), flyttar du dem till ett separat innehållspaket, som bara installeras på AEM Author genom [att bädda](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html#embeddeds) in det i `all` paketet i en AEM Author-körningsmapp (`<target>/apps/example-packages/content/install.author</target>`).
