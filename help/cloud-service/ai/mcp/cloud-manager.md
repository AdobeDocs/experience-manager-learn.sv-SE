---
title: Cloud Manager MCP Server
description: Med Cloud Manager MCP Server kan AEM-utvecklare köra pipelines, felsöka och komma åt Cloud Manager-verktyg direkt i utvecklingsmiljön - ingen kontextväxling.
version: Experience Manager as a Cloud Service
role: Admin, Developer
level: Intermediate, Experienced
doc-type: Technical Video
duration: 97
last-substantial-update: 2026-03-02T00:00:00Z
jira: KT-20425
exl-id: 0811742a-7f1b-4659-8ab0-982ef1527483
source-git-commit: e630a91aa965df4d1222d7949a61dc0fa6f2865b
workflow-type: tm+mt
source-wordcount: '74'
ht-degree: 0%

---

# Cloud Manager MCP Server

Med Cloud Manager MCP Server kan AEM-utvecklare köra pipelines, felsöka och komma åt Cloud Manager-verktyg direkt i utvecklingsmiljön - ingen kontextväxling.

>[!AVAILABILITY]
>
> Cloud Manager MCP Server är nu i betaversion. Om du vill begära åtkomst skickar du ett e-postmeddelande till [aemcs-mcp-feedback@adobe.com](mailto:aemcs-mcp-feedback@adobe.com) med en beskrivning av ditt användningsfall.

>[!VIDEO](https://video.tv.adobe.com/v/3480346/?captions=swe&learn=on&enablevpops)
