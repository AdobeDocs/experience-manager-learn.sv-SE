...
2. Navigera till `ui.apps/src/main/content/jcr_root/apps/wknd-spa-react/components/card` och visa `.content.xml`-filen.
   ...
   Egenskapen `sling:resourceSuperType` pekar på `wknd-spa-react/components/image` som anger att komponenten `Card` kommer att ärva alla funktioner från komponenten WKND SPA Image.
...
6. I IDE-växeln till `ui.frontend`-modulen navigerar du till `ui.frontend/src/components/Card`:
...
8. Inspect metoden `get imageContent()`:
   ...
   I det här exemplet har vi valt att återanvända den befintliga React Image-komponenten `Image` genom att helt enkelt skicka `this.props` från `Card`-komponenten. Senare i självstudiekursen kommer metoden `get bodyContent()` att implementeras för att visa en titel, ett datum och ett anrop till åtgärdsknappen.
...
2. Navigera till SPA sidmall på [http://localhost:4502/editor.html/conf/wknd-spa-react/settings/wcm/templates/spa-page-template/structure.html](http://localhost:4502/editor.html/conf/wknd-spa-react/settings/wcm/templates/spa-page-template/structure.html).
...
2. I `Edit`-läget lägger du till komponenten `Card` i `Layout Container`:
   ...
   ![Infoga ny komponent](assets/extend-component/insert-card-component.png)
...
6. Uppdatera fliken **Resursmetadata** om du vill lägga till värden för **Alternativ text** och **Bildtext**.
   ...
   Inga ytterligare ändringar visas efter att dialogrutan har uppdaterats. Om du vill visa de nya fälten för React Component (React Component) måste du uppdatera Sling Model för `Card`-komponenten.

7. Öppna en ny flik och gå till [CRXDE-Lite](http://localhost:4502/crx/de/index.jsp#/content/wknd-spa-react/us/en/home/jcr%3Acontent/root/responsivegrid/card). Inspect innehållsnoderna under `/content/wknd-spa-react/us/en/home/jcr:content/root/responsivegrid` för att hitta `Card`-komponentinnehållet.
...
2. Lägg till följande metoder i gränssnittet:
   ...
   Dessa metoder visas via JSON-modellens API och skickas till React-komponenten.
...
Nu när JSON-modellen har fyllts i med nya egenskaper för `ctaLinkURL`, `ctaText`, `cardTitle` och `cardLastModified` kan vi uppdatera React-komponenten för att visa dessa.
...
4. Lägg till en metod för `get lastModifiedDisplayDate()` för att omforma `this.props.cardLastModified` till en lokaliserad sträng som representerar datumet.
...
5. Uppdatera `get bodyContent()` så att `this.props.cardTitle` visas och använd de metoder som har skapats i föregående steg:
...
6. Sass-regler har redan lagts till `Card.scss` för att formatera titeln, anropa åtgärden och det senast ändrade datumet. Inkludera dessa format genom att lägga till följande rad i `Card.js` längst upp i filen:
   ...
   >[!NOTE]
   >
   > Du kan visa den färdiga [Reaktionskortskomponentkoden här](https://github.com/adobe/aem-guides-wknd-spa/blob/React/extend-component-solution/ui.frontend/src/components/Card/Card.js).
