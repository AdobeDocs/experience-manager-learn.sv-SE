---
title: Skapa en anpassad komponent | Komma igång med AEM SPA-redigerare och vinkelrät
...
4. Under `custom-component` mappen skapar du en annan mapp med namnet `_cq_dialog`.
...
3. Uppdatera `CustomComponent.java` så att det utökar `ComponentExporter` gränssnittet:
   ...
   Gränssnittet innehåller en enda get-metod `CustomComponent` `getMessage()`. Detta är den metod som kommer att visa värdet för författardialogrutan via JSON-modellen. Endast get-metoder med tomma parametrar `()` exporteras i JSON-modellen.
...
6. Lägg till `getExportedType()` metoden i `CustomComponentImpl` klassen för att returnera komponentresurstypen:
   ...
   Den här metoden krävs vid implementering av `ComponentExporter` gränssnittet och visar den resurstyp som tillåter mappningen till vinkelkomponenten.
...
1. Öppna filen i `ui.frontend` modulen `ui.frontend/src/app/components/custom/custom.component.ts`
2. Observera `@Input() message: string;` raden. Det omformade versalvärdet förväntas mappas till den här variabeln.
...
5. Öppna `custom.component.css` och lägg till följande regel:
   ...
   För att platshållaren för AEM Editor ska kunna visas korrekt när komponenten är tom `:host-context` eller så `<div>` måste en annan ställas in på `display: block;`.
...
1. Verifiera registreringen av den nya Sling-modellen genom att gå till [http://localhost:4502/system/console/status-slingmodels](http://localhost:4502/system/console/status-slingmodels).
   ...
   Du bör se de två raderna ovan som anger att komponenten `CustomComponentImpl` är associerad med `wknd-spa-angular/components/custom-component` komponenten och att den är registrerad via Sling Model Exporter.
...
5. Visa JSON-modellen genom att gå till [http://localhost:4502/content/wknd-spa-angular/us/en.model.json](http://localhost:4502/content/wknd-spa-angular/us/en.model.json). Search for `wknd-spa-angular/components/custom-component`:
